...
import { History, Package, Wallet, Copy, Check, Eye, CreditCard, Clock } from "lucide-react"
...
export default function HistoryPage() {
    ...
    const [historyData, setHistoryData] = useState<{ products: HistoryItem[], deposits: HistoryItem[] }>({ products: [], deposits: [] })
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            try {
                setHistoryData({ products: [], deposits: [] });
            } catch (error) {
    ...
    const renderCodeCell = (item: any) => {
        if (!("code" in item)) return null;

        // Only show code for Completed status
                ...
                <CardContent className="p-0">
                    <div className="hidden md:block">
                        <Table>
                            ...
                            <TableBody>
                                {(activeTab === "products" ? historyData.products : historyData.deposits).map((item) => (
                                    <TableRow key={item.id} className="border-border/50 hover:bg-muted/30">
                                        ...
                                        <TableCell className="text-right py-2 pr-4">
                                            <Badge
                                                variant="outline"
                                                className={`text-[10px] h-5 px-1.5 font-normal border ${item.status === "Completed" ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20 dark:text-emerald-400" :
                                                    item.status === "Pending" ? "bg-amber-500/10 text-amber-600 border-amber-500/20 dark:text-amber-400" :
                                                        item.status === "Processing" ? "bg-blue-500/10 text-blue-600 border-blue-500/20 dark:text-blue-400" :
                                                            item.status === "Refunded" ? "bg-indigo-500/10 text-indigo-600 border-indigo-500/20 dark:text-indigo-400" :
                                                                item.status === "Canceled" ? "bg-gray-500/10 text-gray-600 border-gray-500/20 dark:text-gray-400" :
                                            ...
                                            {item.status === "Pending" && "expiresAt" in item && (
                                                <Button size="sm" className="ml-2 h-5 px-2 text-[9px] rounded-md gap-1" onClick={() => toast.info("Fitur ini segera hadir!")}>
                        ...
                        {(activeTab === "products" ? historyData.products : historyData.deposits).map((item) => (
                            <Card key={item.id} className="overflow-hidden border-border/50 shadow-sm py-0 gap-0">
                                <div className="p-3 space-y-3">
                                    ...
                                    {/* Very Bottom: Product Code (Only for products with Completed status) */}
                                    {"code" in item && item.status === "Completed" && (
